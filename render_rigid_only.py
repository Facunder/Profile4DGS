...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, cam_type, dataset_name):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        time_sub_1 = time()
        render_pkg = render(view, gaussians, pipeline, background,cam_type=cam_type, frame_id=idx, static_mask=None, ref_pos_bias=None, ref_scale_bias=None, ref_rot_bias=None)
        torch.cuda.synchronize()
        time_sub_2 = time()
        ...
        #     static_mask = torch.where((norm_pos_change < 0.0008) & (relative_scale_change < 0.03) & (rot_qua_change < 0.2), torch.ones_like(static_mask, device="cpu"), torch.zeros_like(static_mask, device="cpu"))
        #     pos_static = torch.where(norm_pos_change < 0.0008, torch.ones_like(static_mask, device="cpu"), torch.zeros_like(static_mask, device="cpu"))
        #     scale_static = torch.where(relative_scale_change < 0.03, torch.ones_like(static_mask, device="cpu"), torch.zeros_like(static_mask, device="cpu"))
...
def render_sets(dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video: bool, dataset_name: str):
    with torch.no_grad():
        ...
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False, resolution_scales=[1.0], load_coarse=False)
